  ...
  <header class="header">
    ...
    <div class="navbar">
      <div class="navbar-left-container">
        <a href="/">
          <img class="navbar-brand-logo" src="/static/logo.jpg" style="width: 150px">
        </a>
      </div>
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          <span class="navbar-link"  v-if="nickName">欢迎你，{{nickName}}</span>
          ...
          <div class="confirm-tips">
            ...
            <ul>
              <li class="regi_form_input">
                ...
                <input type="text" tabindex="1" name="loginname" v-model="loginForm.userName" class="regi_login_input regi_login_input_left" placeholder="User Name" data-type="loginname">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i>
                <input type="password" tabindex="2"  name="password" v-model="loginForm.userPwd" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="Password" @keyup.enter="login">
              </li>
            </ul>
          </div>
    ...
    methods:{
      ...
      getCartCount(){
        axios.get("/api-dev/users/getCartCount").then(res=>{
          var res = res.data;
          this.$store.commit("SET_COUNT",res.result);
          this.$cookies.set('carCount', res.result)
        });
      },
      //登录
      login(){
        if(this.loginForm.userName==''||this.loginForm.userPwd==''){
          this.errorTipText='请填写用户名和密码';
          this.errorTip=true;
          return;
        }
        axios.post('/api-dev/users/login',{
          ...
          if(res.data.status=='1'){
            this.errorTipText='用户名或密码错误';
            this.errorTip=true;//显示报错
          }
          if(res.data.status=='0') {
            this.errorTip=false;//关闭报错
            //将状态存储到vuex中
            this.$store.commit('SET_USERID',res.data.result.userId);
            this.$store.commit('SET_NAME', res.data.result.userName);
            //把购物车数量放到cookie中
            this.getCartCount();
            //关闭模态框
      ...
      //登出
      logOut(){
        axios.post('/api-dev/users/logout').then((res)=>{
          if(res.data.status=='0'){
            this.$store.commit('SET_USERID','');
            this.$store.commit('SET_NAME', '');
            this.$store.commit("SET_COUNT",0);
