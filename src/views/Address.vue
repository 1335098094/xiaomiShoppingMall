      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressListFilter"  style="text-align: left" :class="{'check':checkIndex==index}" @click="checkIndex=index;selectId=item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-set-default" >
                    <a href="javascript:;" class="addr-set-default-btn" v-show="!item.isDefault" @click="serDefault(item)"><i>Set default</i></a>
                  </div>
                  <div class="addr-opration addr-default" v-show="item.isDefault">Default address</div>
                </li>
                <li class="addr-new" @click="addShow()">
          ...
          <div class="next-btn-wrap">
            <a class="btn btn--m btn--red" @click="next()">Next</a>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="md-modal-inner">
        ...
        <div class="md-content">
          <div class="confirm-tips">
            ...
            <ul>
              <p class="input_t"><span>*</span>姓名：</p>
              <li class="regi_form_input">
                <input type="text"   v-model="query.userName" class="regi_login_input regi_login_input_left"  style="left:10px" placeholder="请输入姓名">
              </li>
              <p class="input_t"><span>*</span>号码：</p>
              <li class="regi_form_input noMargin">
                <input type="text"    v-model="query.tel" class="regi_login_input regi_login_input_left login-input-no input_text" style="left:10px" placeholder="请输入电话号码">
              </li>
              <p class="input_t"><span>*</span>邮政编号：</p>
              <li class="regi_form_input noMargin">
                <input type="text"    v-model="query.postCode" class="regi_login_input regi_login_input_left login-input-no input_text" style="left:10px" placeholder="请输入邮政编号">
              </li>
              <p class="input_t"><span>*</span>地址：</p>
              <li class="regi_form_input noMargin textarea" style="height: auto;border:1px solid #ccc">
                <textarea  rows="3" cols="20"  placeholder="请输入地址" v-model="query.streetName" style="border:0px;width:100%"></textarea>
              </li>
            </ul>
          </div>
          <div style="text-align: left" class="setDefault">
            <input type="checkbox" id="checkbox" v-model="query.isDefault"/>
            <label for="checkbox" class="lables"></label><span class="span">设置为默认地址</span>
          </div>
          <div class="login-wrap">
            <a href="javascript:;" class="btn-login" @click="addAddress()">确认</a>
          </div>
        </div>
      </div>
    </div>
  ...
  import axios from 'axios'
    export default {
      ...
      methods:{
        //获取地址
        initData(){
          axios.get('/api-dev/users/address').then((res)=>{
            if(res.data.status=='1'){
              this.$alert('提示',res.data.msg)
            }else{
              this.addressList=res.data.result;//数组赋值
              if(res.data.result.length!=0){
                ...
                //设置默认地址情况下
                this.addressList.forEach((item,index)=>{ //默认选择地址高亮
                  if(item.isDefault){
        ...
        //设置为默认
        serDefault(items){
          axios.post('/api-dev/users/setDefault',{
        ...
        //提交地址
        addAddress(){
           if(this.query.userName&&this.query.postCode&&this.query.streetName&&this.query.tel){
             this.errorTip=false;
             this.loginModalFlag=false;
             axios.post('/api-dev/users/addAddress',{query:this.query}).then((res)=>{
             ...
             this.errorTipText="请填写必填内容";
             this.errorTip=true
           }
        },
        //删除地址
        delAddress(addressId){
          axios.post('/api-dev/users/delAddress',{addressId}).then((res)=>{
            if(res.data.status=='0'){
              this.initData()
            }else{
              this.$alert('提示','删除失败')
        ...
        //生成订单
        next(){
          this.$router.push({path: '/orderConfirm', query: { addressId: this.selectId }});
        }
      }
    }
</script>
