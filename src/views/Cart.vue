<template>
  <div>
              ...
              <li v-for="(item,index) in cartList">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delCart(item,index)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;">
                  <span class="checkbox-btn item-check-btn" v-bind:class="{'check':isCheckAll}"  @click="checkAll()">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                总费用: <span class="total-price">{{allPrice | currency('￥')}}</span>
              </div>
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':checkedTotal==0}" @click="goAddress()">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <model :mdShow="mdShowConfirm" @close="close()">
      <p slot="message" style="font-size: 16px;font-weight: bold">
        确认删除？
      </p>
      <a slot="btns" href="javascript:;" class="btn btn--m" style="margin-top:0px;margin-right: 40px" @click="confirmdelCarts()">确认</a>
      <a slot="btns" href="javascript:;" class="btn btn--m" style="margin-top:0px" @click="close()">关闭</a>
    </model>
      ...
      computed:{
          isCheckAll(){
              if(this.cartList.length>0){
                return　this.cartList.length==this.checkedTotal
              }
          },
          checkedTotal:{
          get(){
      ...
      methods:{
        ...
        confirmdelCarts(){
          axios.post('/api-dev/users/delCart',{
            productId:[this.cartGoods.productId]
          }).then((res)=>{
             if(res.data.status=='0'){
               this.mdShowConfirm=false;
               this.cartList.splice(this.cartIndex,1);
               var num=parseInt(this.cartGoods.productNum);
               this.$store.commit('SET_UPDATECOUNT',-num);
             }else{
               this.$alert('提示',"删除失败")
             }
          }).catch((err)=>{
        ...
        editCart(flag,item){
            ...
            axios.post('/api-dev/users/changeCart',{
        ...
        checkAll(){
          ...
          this.cartList.forEach(item=>{
            item.checked=flag==true?'0':'1'
          });
          var check=flag==true?'0':'1';
          axios.post('/api-dev/users/changeAllCart',{
            'checked':check
          }).then((res)=>{
            if(res.data.status=='1'){
              this.$alert('提示',res.data.msg)
